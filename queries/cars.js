...
async function getCarsByND(name, year) {
  const somecar = await db
    .any("select * from cars where name = $1 and model_year = $2", [name, year])
    .then((res) => res)
    .catch((e) => {
      return e;
    });
...
async function getCarsByName(name) {
  const somecar = await db
    .any("select * from cars where name ILIKE $1 ORDER BY model_year DESC", [
      name,
    ])
    .then((res) => res)
    .catch((e) => {
...
async function deleteRow(args) {
  try {
    const Del = await db.any(`DELETE FROM cars where id = ${args} RETURNING *`);
    return Del;
  } catch (e) {
    return e;
  }
